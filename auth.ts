...
export class DomestikaAuth {
    ...
    async promptForCredentials(forceUpdate: boolean = false): Promise<void> {
...
DOMESTIKA_CREDENTIALS=${this._credentials_}`;
    ...
    async getCookies(): Promise<Credentials> {
        ...
        if (!await this.validateCredentials()) {
            ...
            if (!await this.validateCredentials()) {
                throw new Error('Could not obtain valid credentials. Please verify your Domestika cookies.');
            }
        }
