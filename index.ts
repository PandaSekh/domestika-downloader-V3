...
function getVideoId(courseUrl: string, unitNumber: number, videoIndex: number): string {
    ...
    try {
        ...
        records.forEach(row => {
            ...
            if (url && unitNumber && videoIndex && status.toLowerCase() === 'completed') {
                ...
                const videoId = getVideoId(normalized.url, parseInt(unitNumber), parseInt(videoIndex));
...
function checkVideoFileExists(
    ...
    const finalDir = cleanPath(path.join(baseDownloadPath, courseTitle || 'Unknown Course', section, unitTitle));
...
function isVideoCompleted(
    ...
    if (courseTitle !== undefined && unitTitle !== undefined && videoTitle !== undefined && section !== undefined) {
        const existingFile = checkVideoFileExists(courseTitle, unitTitle, unitNumber, videoIndex, videoTitle, section);
        if (existingFile) {
            return true;
        }
    }
...
function saveVideoProgress(
    ...
    // Check if this video is already in progress
    ...
    const existingIndex = records.findIndex(row => {
        ...
        if (rowUrl && rowUnitNumber && rowVideoIndex) {
            ...
            const rowVideoId = getVideoId(rowNormalized.url, parseInt(rowUnitNumber), parseInt(rowVideoIndex));
            return rowVideoId === videoId;
        }
        return false;
    });
    ...
    try {
        ...
        const normalizedRecords = records.map(record => ({
            url: record.url || record.URL || record.course_url || record.courseUrl || '',
            courseTitle: record.courseTitle || record.course_title || '',
        ...
        const csvContent = csvStringifySync(normalizedRecords, {
            header: true,
            columns: ['url', 'courseTitle', 'unitNumber', 'unitTitle', 'videoIndex', 'videoTitle', 'status', 'timestamp']
        });
    ...
    try {
        ...
        for (let i = 0; i < coursesToProcess.length; i++) {
            ...
            try {
                ...
                await scrapeSite(course.url, course.subtitles, auth, course.downloadOption, course.courseTitle, completedVideos);
...
async function scrapeSite(
    ...
    if (units.length === 0) {
        ...
        if (answer.updateCookies) {
            ...
            return scrapeSite(courseUrl, subtitle_langs, await domestikaAuth.getCookies(), downloadOption, courseTitle, completedVideos);
        } else {
            ...
            if (typeof selection === 'string' && selection.startsWith('unit_')) {
                ...
                if (unit) {
                    for (let i = 0; i < unit.videoData.length; i++) {
                        ...
                        if (isVideoCompleted(courseUrl, unit.unitNumber, videoIndex, completedVideos, courseTitle, unit.title, vData.title, vData.section)) {
                            console.log(`⏭️  Skipping already downloaded video: ${vData.title}`);
                            continue;
                        }
                        await downloadVideo(
                            vData,
                ...
                // Check if video is already completed (including file system check)
                if (isVideoCompleted(courseUrl, selection.unit.unitNumber, selection.index, completedVideos, courseTitle, selection.unit.title, selection.videoData.title, selection.videoData.section)) {
                    console.log(`⏭️  Skipping already downloaded video: ${selection.videoData.title}`);
                } else {
                    await downloadVideo(
                        selection.videoData,
                        courseTitle,
    ...
    for (const unit of allVideos) {
        for (let i = 0; i < unit.videoData.length; i++) {
            const vData = unit.videoData[i];
            if (isVideoCompleted(courseUrl, unit.unitNumber, i + 1, completedVideos, courseTitle, unit.title, vData.title, vData.section)) {
                skippedCount++;
            }
        }
    }
            ...
            downloadPromises.push(
                (async (): Promise<boolean> => {
                    try {
                        ...
                        await downloadVideo(vData, courseTitle, unit.title, videoIndex, subtitle_langs, unit.unitNumber, multiBar, courseUrl, completedVideos);
                        downloadedCount++;
                        console.log(`\nCompleted ${downloadedCount} of ${totalVideos - skippedCount} new videos`);
                        return true;
                    } catch (error) {
                        const err = error as Error;
    ...
    if (downloadedCount === 0 && skippedCount === 0) {
        ...
        console.log(`\nProcess completed. Downloaded ${downloadedCount} new video(s), skipped ${skippedCount} already downloaded video(s) (${downloadedCount + skippedCount} of ${totalVideos} total)`);
    }
...
async function downloadVideo(
    ...
    try {
        ...
        const existingFile = checkVideoFileExists(courseTitle, unitTitle, unitNumber, index, vData.title, vData.section);
        if (existingFile) {
            console.log(`\n✅ Video file already exists: ${existingFile}`);
            console.log(`⏭️  Skipping download and marking as completed in progress...`);
